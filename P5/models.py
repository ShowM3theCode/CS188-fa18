...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        return nn.AddBias(nn.Linear(x1, self.w2), self.b2)
    ...
    def train(self, dataset):
        ...
        loss = float('inf')
        while loss > 0.01:
            gw1, gb1, gw2, gb2 = nn.gradients(self.get_loss(nn.Constant(dataset.x), nn.Constant(dataset.y)),
                              [self.w1, self.b1, self.w2, self.b2])
            self.w1.update(gw1, -0.01)
            self.b1.update(gb1, -0.01)
            self.w2.update(gw2, -0.01)
            self.b2.update(gb2, -0.01)
            loss = nn.as_scalar(self.get_loss(nn.Constant(dataset.x),nn.Constant(dataset.y)))
        loss = -float('inf')
        for x, y in dataset.iterate_forever(100):
            gw1, gb1, gw2, gb2 = nn.gradients(self.get_loss(x, y),
                                              [self.w1, self.b1, self.w2, self.b2])
            self.w1.update(gw1, -0.01)
            self.b1.update(gb1, -0.01)
            self.w2.update(gw2, -0.01)
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < 0.97:
            for x, y in dataset.iterate_once(10):
                ...
                gw1, gb1, gw2, gb2, gw3, gb3 = nn.gradients((loss),
                              [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
                self.w1.update(gw1, -0.01)
                self.b1.update(gb1, -0.01)
                self.w2.update(gw2, -0.01)
                self.b2.update(gb2, -0.01)
                self.w3.update(gw3, -0.01)
                self.b3.update(gb3, -0.01)
    ...
    def train(self, dataset):
        ...
        for i in range(21):
            for x, y in dataset.iterate_once(60):
                grad_wrt_W1, grad_wrt_b1, grad_wrt_W2, grad_wrt_b2, grad_wrt_W1_hidden, grad_wrt_b1_hidden, grad_wrt_W2_hidden, grad_wrt_b2_hidden, grad_wrt_W_final, grad_wrt_b_final = nn.gradients(
                    self.get_loss(x, y),
                    [self.W1, self.b1, self.W2, self.b2, self.W1_hidden, self.b1_hidden, self.W2_hidden, self.b2_hidden,
                ...
                self.W1_hidden.update(grad_wrt_W1_hidden, -0.1)
                self.b1_hidden.update(grad_wrt_b1_hidden, -0.1)
                self.W2_hidden.update(grad_wrt_W2_hidden, -0.1)
                self.b2_hidden.update(grad_wrt_b2_hidden, -0.1)
                self.W_final.update(grad_wrt_W_final, -0.1)
                self.b_final.update(grad_wrt_b_final, -0.1)
