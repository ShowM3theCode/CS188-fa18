    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                cost = 1
                if nextState[0] in self.corners_list:
                    index = self.corners_list.index(nextState[0])
                    nextState[index + 1] = 1
...
def cornersHeuristic(state, problem):
    ...
    A heuristic for the CornersProblem that you defined.
      ...
      problem:
The CornersProblem instance for this layout.
    ...
    for val in state[1:]:
        if val == 0:
            ans = max(ans, util.manhattanDistance(currentPosition, corners[i]))
        i += 1
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        if problem.isGoalState(startPosition): return Directions.STOP
        ...
        while not fringe.isEmpty():
            ...
            if currentState in expandedNodes: continue
            expandedNodes.add(currentState)
            successors = problem.getSuccessors(currentState)
            for nex in successors:
                if nex[0] in expandedNodes: continue
